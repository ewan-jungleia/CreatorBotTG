...
async function refineReadme(brief, base){
  ...
  const chat = await openai.chat.completions.create({
    model: MODEL,
    messages: [{role:"system",content:"Tu écris des README concis pour bots Telegram sur Vercel."},{role:"user",content:`Brief:\n${brief}\n\nREADME:\n${base}\n\nRends-le plus clair et actionnable.`}],
    temperature:0.2
  });
...
async function onCallbackQuery(cb){
  ...
  if(data==="confirm_project"){
    ...
    const z2=new JSZip(), z1=await JSZip.loadAsync(zipBuffer); for(const e of Object.keys(z1.files)){ const f=z1.files[e]; const c=await f.async("uint8array"); if(e==="README.md") z2.file(e,new TextEncoder().encode(refined)); else z2.file(e,c); }
