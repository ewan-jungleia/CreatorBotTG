...
async function createProjectFromTmp(chatId, userId){
  ...
  const project = { id: pid, title: tmp.title, version: 'v1', status: 'draft', budget: { capCents: tmp.capCents||0, alertStepCents: tmp.alertStepCents||0 }, prompt: tmp.prompt || '' };
...
async function listProjects(chatId, userId){
  const keys = keysForUser(userId);
  const list = (await getJSON(keys.projectsList)) || [];
  if (!list.length){ await reply(chatId, 'Aucun projet. Lance un nouveau projet.', kb([[{ text:'🆕 Nouveau projet', callback_data:'act:new' }],[{ text:'⬅️ Retour', callback_data:'act:menu' }]])); return; }
...
async function handleStart(chatId, userId){ await ensureBudgetDefaults(userId); await reply(chatId, 'CreatorBot-TG en ligne ✅\nChoisis une action :', mainMenu()); }
